...
internal tonic/hyper h2 Raft append handling and the then-current `rmp_serde`
RPC decode path, matching that proposal fanout. The RPC/log serialization path
has since moved to typed protobuf messages. The runtime/Raft adapter now
coalesces same-group append-batch
